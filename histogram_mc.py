...
while True:
    ...
    for header in data:
        #print(column)
        total_min = round(float(min(data[header]))*100,2)
        total_max = round(float(max(data[header]))*100,2)
    ...
    fig = plt.gcf()
    print(f"{N_of_class} -> sum={np.sum(N_of_class["Hardware"])}")
    plt.show()
